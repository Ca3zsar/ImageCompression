...
class Decoder(tf.keras.models.Model):
    def __init__(self, batch_size, binary_size, patch_size):
        ...
        self.custom_layers.append(layers.Dense(patch_size*patch_size*3, activation='tanh', trainable=True))
...
class LSTM_Autoencoder(tf.keras.models.Model):
    ...
    def call(self, inputs, training=True):
        x = inputs
        self.encoder_state = self.encoder.init_state()
        self.decoder_state = self.decoder.init_state()

        for i in range(self.steps):
            bits, self.encoder_state = self.encoder((x, self.encoder_state))
            x, self.decoder_state = self.decoder((bits, self.decoder_state))
