...
class SynthesisTransform(tf.keras.Sequential):
    ...
    def __init__(self, num_filters):
        ...
        self.add(tfc.SignalConv2D(
            num_filters // 2, (5, 5), name='layer0', corr=False, strides_up=2,
...
class EndToEndModel(tf.keras.Model):
    ...
    def call(self, x, training):
        ...
        distortion_l2 = tf.reduce_mean(tf.math.squared_difference(x, x_tilde))
        distortion_l1 = tf.compat.v1.losses.absolute_difference(x,
                                              x_tilde,
                                              reduction=tf.compat.v1.losses.Reduction.MEAN)
        distortion = distortion_l1 *0.5 + distortion_l2 * 0.5
