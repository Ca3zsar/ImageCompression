...
class EndToEndModel(tf.keras.Model):
    ...
    def __init__(self, lmbda, num_filters, latent_dims, prior_values=None):
        ...
        # Exponential moving average of the distortion
        self.ema = tf.train.ExponentialMovingAverage(decay=args["avg_decay"])
        self.running_dissimilarity = tf.Variable(0.0, trainable=False)
    ...
    def call(self, x, training):
        ...
        dissimilarity = 0.5 * (1 - tf.image.ssim(x, x_tilde, max_val=255))
        dissimilarity = tf.cast(dissimilarity, tf.float32)
        self.running_dissimilarity.assign(
            tf.squeeze(self.running_dissimilarity)
        )

        # Update the exponential moving average
        self.ema.apply([self.running_dissimilarity])
        baseline = self.ema.average(self.running_dissimilarity)

        # Compute the distortion
        distortion = tf.compat.v1.losses.absolute_difference(
            x, x_tilde, reduction=tf.compat.v1.losses.Reduction.MEAN
        )

        distortion = dissimilarity/(baseline + args["epsilon"]) * distortion
...
def train(model=None):
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4, clipnorm=0.5))
    ...
    model.fit(
        ...
        callbacks=[
            ...
            tf.keras.callbacks.ModelCheckpoint(
                ...
                mode="min",
                verbose=1,
                initial_value_threshold=41629652.0
            ),
            reduceLRonPlateau,
        ],
        verbose=1,
