...
def compute_benchmark(
    ...
    dataset_name = data_directory.split("/")[-2]
    
    if save:
        os.makedirs(f"results/{save_path}/{dataset_name}",exist_ok=True)
...
#         result = tf.expand_dims(result, axis=-1)
        ...
        ssim = tf.image.ssim(image_copy, result_copy, max_val=1)
        ms_ssim = tf.image.ssim_multiscale(image_copy, result_copy, max_val=1)
        psnr = tf.image.psnr(image_copy, result_copy, max_val=1)
        ...
        if save:
            write_image(tf.saturate_cast(tf.round(result_copy * 255), tf.uint8), f'results/{save_path}/{dataset_name}/{image_path.split("/")[-1]}')
