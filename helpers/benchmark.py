...
def reconstruct(image: np.ndarray, compress_function, decompress_function) -> tuple[np.ndarray, np.ndarray]:
    patch_size = 32
...
def plot_image(image, result, stats):
    ...
    plt.text(0, 0.4, f"MS-SSIM: {stats['ms_ssim'].numpy():.4f}", fontsize=12, color='red')
    plt.text(0, 0.6, f"PSNR: {stats['psnr'].numpy():.4f}", fontsize=12, color='red')
    plt.text(0, 0.8, f"Compression Ratio: {stats['compression_ratio'].numpy():.4f}", fontsize=12, color='red')
    plt.axis("off")
...
def compute_benchmark(data_directory: str, reconstruct_function: Callable, network: tf.keras.Model, plot: bool = True, save: bool = False, save_path: str = None):
    ...
    for image_path in benchmark_images:
        ...
        result, strings = reconstruct_function(image_copy, network.compress, network.decompress)
