...
args = {
    ...
    "latent_dims" : 128,
    "lambda_value" : 0.005,
    "perceptual_lambda" : 0.0001,
    "num_epochs" : 57,
    "steps_per_epoch" : 1000,
    "checkpoint_dir" : "models/checkpoints/",
    "model_dir" : "models/",
    "model_name" : "end_to_end_v24",
    "field" : "hr"
}
...
class EndToEndModel(tf.keras.Model):
    ...
    def __init__(self, lmbda, num_filters, latent_dims, prior_values = None):
        ...
        self.lmbda = lmbda
        self.perceptual_lambda = args["perceptual_lambda"]
        ...
        self.entropy_model = tfc.ContinuousBatchedEntropyModel(self.prior, coding_rank=3, compression=True, laplace_tail_mass=(2 ** -9))
        
        vgg = VGG16(weights='imagenet', include_top=False)
        layer_names = ['block3_conv3', 'block4_conv3']
        layers = [vgg.get_layer(name).output for name in layer_names]
        
        self.perceptual_model = tf.keras.Model(inputs=vgg.input, outputs=layers)
        self.perceptual_model.trainable = False  # Freeze the VGG16 model weights
    ...
    def call(self, x, training):
        ...
        ssim_loss = (1 - tf.image.ssim(x, x_tilde, max_val=255)) * 10 + 1e-8
        distortion = distortion * ssim_loss
        distortion = tf.cast(distortion, tf.float32)
        
        vgg_true = self.perceptual_model(x)
        vgg_reconstruct = self.perceptual_model(x_tilde)

        perceptual_loss = 0
        for true_feature, pred_feature in zip(vgg_true, vgg_reconstruct):
            perceptual_loss += K.mean(K.square(true_feature - pred_feature))
        
        # Compute the rate-distortion loss
        loss = bpp + self.lmbda * distortion + self.perceptual_lambda * perceptual_loss

        return loss, bpp, ssim_loss, distortion, perceptual_loss
    
    def train_step(self, x):
        with tf.GradientTape() as tape:
            loss, bpp, dissimilarity, distortion, perceptual = self(x, training=True)
        ...
        self.perceptual.update_state(perceptual)
        self.distortion.update_state(distortion)

        return {m.name: m.result() for m in [self.loss, self.bpp, self.dissimilarity, self.distortion, self.perceptual]}
    
    def test_step(self, x):
        loss, bpp, dissimilarity, distortion, perceptual = self(x, training=False)
        self.loss.update_state(loss)
        self.bpp.update_state(bpp)
        self.perceptual.update_state(perceptual)
        self.dissimilarity.update_state(dissimilarity)
    ...
    def compile(self, **kwargs):
        ...
        self.perceptual = tf.keras.metrics.Mean(name="perceptual")
...
def train(model=None):
    ...
    model.compile(
        optimizer=tf.keras.optimizers.AdamW(learning_rate=1e-3, clipnorm=0.75)
    )

    train_dataset = get_dataset("div2k", "train", args)
    validation_dataset = get_dataset("div2k", "validation", args)
